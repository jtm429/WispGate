...
class SessionCryptoTest {
    ...
    fun matchesPythonSessionFixtureInBothDirections() {
        ...
        val session = PeerSession("fixture-session", "android-endpoint-uuid", "prime-wisp", keys, 100_000L, androidSide = true)
        val envelope = session.encrypt(JSONObject().put("action", "state_request"), 101_000L)
        assertEquals("35795103fff46cace67a89b589f4c1bc5ed059b8f8887d13a264232955cfd040bd904a330a29ec4b5485", SessionCrypto.decode64(envelope.getString("ciphertext")).toHex())
    ...
    fun rejectsReplayAndAbsoluteExpiration() {
        val keys = SessionCrypto.deriveKeys(master, "fixture-session")
        val sender = PeerSession("fixture-session", "android-endpoint-uuid", "prime-wisp", keys, 10_000L, androidSide = true)
        val receiver = PeerSession("fixture-session", "prime-wisp", "android-endpoint-uuid", keys, 10_000L, androidSide = false)
