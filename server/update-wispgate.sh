...
sudo -n -u wisp -- git -c safe.directory="$repo" -C "$repo" fetch origin main
sudo -n -u wisp -- git -c safe.directory="$repo" -C "$repo" merge --ff-only origin/main
printf '[%s] checked_out=%s\n' "$(date --iso-8601=seconds)" "$(git -c safe.directory="$repo" -C "$repo" rev-parse --short HEAD)"
