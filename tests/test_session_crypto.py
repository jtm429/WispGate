...
CIPHERTEXT = "35795103fff46cace67a89b589f4c1bc5ed059b8f8887d13a264232955cfd040bd904a330a29ec4b5485"
...
def test_session_crypto_matches_cross_language_fixture() -> None:
    ...
    assert session_aad(SESSION_ID, "android-endpoint-uuid", "prime-wisp", 7).decode() == (
        '{"recipient":"prime-wisp","sender":"android-endpoint-uuid","sequence":7,'
        '"session_id":"fixture-session","type":"session_envelope","version":1}'
    )
    sender = PeerSession(SESSION_ID, "android-endpoint-uuid", "prime-wisp", android_key, wisp_key, created_at=100.0, android_side=True)
    envelope = sender.encrypt({"action": "state_request"}, now=101.0)
    assert base64.urlsafe_b64decode(envelope["ciphertext"] + "==").hex() == CIPHERTEXT
    receiver = PeerSession(SESSION_ID, "prime-wisp", "android-endpoint-uuid", android_key, wisp_key, created_at=100.0, android_side=False)
...
def test_session_crypto_rejects_replay_tampering_and_absolute_expiration() -> None:
    android_key, wisp_key = derive_session_keys(MASTER, SESSION_ID)
    sender = PeerSession(SESSION_ID, "android-endpoint-uuid", "prime-wisp", android_key, wisp_key, created_at=10.0, android_side=True)
    receiver = PeerSession(SESSION_ID, "prime-wisp", "android-endpoint-uuid", android_key, wisp_key, created_at=10.0, android_side=False)
